...
$financialOps=new FinancialOperationService($pdo);
...
try{
 if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['action'])){verify_csrf();$action=(string)$_POST['action'];
  if(in_array($action,['open_cash','open_table','add_item','qty_plus','qty_minus','cancel_item','complimentary','complimentary_selected','save_note','discount','move_table','take_payment','close_cash'],true)){$businessDay=$businessDayService->requireOpen();}
  ...
  if($action==='discount'){
   ...
   $discountType=(string)($_POST['discount_type']??'amount');
   $discountValue=max(0,(float)($_POST['discount_value']??0));
   $reason=mb_substr(trim((string)($_POST['discount_reason']??'')),0,190);
   $note=mb_substr(trim((string)($_POST['discount_note']??'')),0,255);
   $financialOps->applyDiscount($sid,$discountType,$discountValue,$reason,$note);
   redirect('./?session='.$sid.'&pay=1');
  }
  if($action==='complimentary_selected'){
   $sid=(int)$_POST['session_id'];
   $raw=json_decode((string)($_POST['product_selection']??'{}'),true);
   $selection=[];
   if(is_array($raw))foreach($raw as $itemId=>$qty){$selection[(int)$itemId]=(float)$qty;}
   $reason=mb_substr(trim((string)($_POST['complimentary_reason']??'')),0,190);
   $note=mb_substr(trim((string)($_POST['complimentary_note']??'')),0,255);
   $financialOps->complimentarySelected($sid,$selection,$reason,$note);
   redirect('./?session='.$sid.'&pay=1&gifted=1');
  }
...
$areas=$pdo->query("SELECT id,name FROM dining_areas WHERE is_active=1 ORDER BY sort_order,id")->fetchAll();
...
<?php if(isset($_GET['paid'])):?><div class="v4-alert success" data-auto-dismiss>Ödeme başarıyla kaydedildi.</div><?php endif;?><?php if(isset($_GET['gifted'])):?><div class="v4-alert success" data-auto-dismiss>Seçili ürünler ikram olarak işlendi.</div><?php endif;?><?php if($error):?><div class="v4-alert danger"><?=e($error)?></div><?php endif;?>
  ...
  <section class="ch-pay-products ch-pay-ticket">
   ...
   <div class="ch-pay-product-list">
    ...
    <?php foreach($items as $it):?>
     <?php if($it['status']==='active'&&$it['open_quantity']>.009):?>
      <button type="button" class="ch-pay-product" data-u-item data-id="<?=$it['id']?>" data-price="<?=number_format((float)$it['unit_price'],2,'.','')?>" data-max="<?=number_format((float)$it['open_quantity'],2,'.','')?>"><span class="qty"><?=e($it['open_quantity'])?></span><span class="name"><?=e($it['product_name'])?><small><?=money((float)$it['unit_price'])?> / adet<?php if($it['paid_quantity']>.009):?> · <?=e($it['paid_quantity'])?> ödendi<?php endif;?></small><?php if($it['item_note']):?><small>📝 <?=e($it['item_note'])?></small><?php endif;?></span><span class="line"><?=money((float)$it['unit_price']*(float)$it['open_quantity'])?></span><span class="picked" data-picked>0</span></button>
     <?php elseif($it['status']==='complimentary'):?>
      <div class="ch-pay-product ch-pay-product--gift"><span class="qty"><?=e($it['quantity'])?></span><span class="name"><?=e($it['product_name'])?><small><?=money((float)$it['unit_price'])?> / adet</small><?php if($it['item_note']):?><small>📝 <?=e($it['item_note'])?></small><?php endif;?></span><span class="line"><b>İKRAM</b><small><?=money((float)$it['unit_price']*(float)$it['quantity'])?></small></span></div>
     <?php endif;?>
    <?php endforeach;?>
   </div>
   ...
   <div class="ch-pay-ticket-summary"><span>Ara Toplam <b><?=money($financial['subtotal'])?></b></span><span>İndirim<?php if(($active['discount_type']??'')==='percent'&&(float)($active['discount_value']??0)>0):?> %<?=e(rtrim(rtrim(number_format((float)$active['discount_value'],2,'.',''),'0'),'.'))?><?php endif;?> <b>− <?=money($financial['discount'])?></b></span><strong>Kalan <em><?=money($financial['remaining'])?></em></strong></div>
  </section>
  <section class="ch-pay-workspace">
   <div class="ch-pay-overview">
    <article><span>Genel Toplam</span><strong><?=money($financial['total'])?></strong></article>
    <article class="paid"><span>Ödenen Toplam</span><strong><?=money($financial['paid'])?></strong></article>
    <article class="remaining"><span>Kalan Tutar</span><strong><?=money($financial['remaining'])?></strong></article>
   </div>
   <nav class="ch-pay-tabs" aria-label="Ödeme türü">
    <button type="button" class="active" data-pay-tab="amount">⌨ Tutar Girerek Ödeme</button>
    <button type="button" data-pay-tab="products">▦ Ürün Bazlı Ödeme</button>
    <button type="button" data-pay-tab="person">♙ Kişi Bazlı Ödeme</button>
   </nav>
   <div class="ch-pay-workbody">
    <section class="ch-pay-center">
     <div class="ch-pay-column-title"><h2>Tutar</h2><small>Ödenecek tutarı belirleyin</small></div>
     ...
     <div class="ch-pay-tools"><button type="button" onclick="discountDialog.showModal()">% İskonto</button><button type="button" data-open-complimentary>🎁 İkram</button><button type="button" data-free-amount>₺ Serbest Tutar</button><a target="_blank" href="../print/?type=bill&session=<?=$selected?>">▤ Ön Hesap Yaz</a></div>
    ...
    <section class="ch-pay-methods">
     <div class="ch-pay-column-title"><h2>Ödeme Yöntemi</h2><small>Tutarı bir veya birden fazla yönteme dağıtın</small></div>
     <div class="ch-pay-method-grid">
     ...
     <div class="ch-pay-payment-actions">
...
<main class="v4-cash-layout <?=!$active?'tables-mode':'sale-mode'?>">
 <?php if(!$active):?>
 <aside class="v4-side-nav"><div class="v4-side-title">SALONLAR</div><button type="button" class="v4-side-link active" data-area="all">Tüm Masalar</button><?php foreach($areas as $area):?><button type="button" class="v4-side-link" data-area="area-<?=(int)$area['id']?>"><?=e($area['name'])?></button><?php endforeach;?></aside>
 <?php else:?>
 <aside class="v4-action-rail"><button class="active">▥<span>Masalar</span></button><button onclick="moveDialog.showModal()">⇄<span>Taşı</span></button><button onclick="discountDialog.showModal()">％<span>İndirim</span></button><a target="_blank" href="../print/?type=bill&session=<?=$selected?>">▤<span>Hesap</span></a><div></div><button onclick="closeCash.showModal()">⌁<span>Kasa</span></button></aside>
 <?php endif;?>
 <?php if(!$active):?>
 <section class="v4-table-main cashier cashier-tables-only ch-cashier-tables"><div class="v4-page-head"><div><h1>Masalar</h1><p>Tahsilat için masa seçin.</p></div><div class="v4-metrics"><div><b><?=count(array_filter($tables,fn($t)=>$t['status']==='empty'))?></b><span>Boş</span></div><div><b><?=count(array_filter($tables,fn($t)=>!empty($t['session_id'])))?></b><span>Dolu</span></div><div><b><?=count($tables)?></b><span>Toplam</span></div></div></div><div class="v4-table-toolbar ch-table-toolbar"><label class="v4-search"><span>⌕</span><input type="search" placeholder="Masa ara..." data-table-search></label><div class="v4-legend"><i class="empty"></i>Boş <i class="open"></i>Dolu <i class="reserved"></i>Rezerve</div></div><div class="v4-table-grid ch-table-grid" data-table-grid><?php foreach($tables as $t):$status=(string)$t['status'];$isOpen=!empty($t['session_id']);$statusLabel=$isOpen?'DOLU':($status==='empty'?'BOŞ':($status==='reserved'?'REZERVELİ':mb_strtoupper($status)));?><article class="v4-table-card <?=$isOpen?'is-open':'is-'.$status?>" <?php if(!empty($t['opened_at'])):?>data-opened="<?=e((string)$t['opened_at'])?>"<?php endif;?> data-area="area-<?=(int)$t['area_id']?>" data-table-id="<?=(int)$t['id']?>" data-session-id="<?=(int)($t['session_id']??0)?>" data-table-name="<?=e(mb_strtolower((string)$t['name']))?>">
 <?php if($isOpen):?><a href="?session=<?=(int)$t['session_id']?>&pay=1" aria-label="<?=e($t['name'])?> tahsilat ekranına geç"><div class="ch-table-card-v2"><div class="ch-table-card-v2__top"><span class="ch-table-card-v2__status"><?=e($statusLabel)?></span><span class="ch-table-card-v2__more" aria-hidden="true">⋮</span></div><div class="ch-table-card-v2__identity"><h3 class="ch-table-card-v2__name"><?=e($t['name'])?></h3><span class="ch-table-card-v2__area"><?=e($t['area_name'])?></span></div><div class="ch-table-card-v2__divider"></div><div class="ch-table-card-v2__metrics"><span class="ch-table-card-v2__metric"><span class="ch-table-card-v2__metric-icon">♙</span><span class="ch-table-card-v2__metric-copy"><b><?=(int)$t['guest_count']?></b><small>Kişi</small></span></span><span class="ch-table-card-v2__metric"><span class="ch-table-card-v2__metric-icon">◷</span><span class="ch-table-card-v2__metric-copy"><b data-open-duration>-- dk</b><small>Bekleme</small></span></span></div><strong class="ch-table-card-v2__footer"><?=money((float)$t['table_total'])?></strong></div></a>
 <?php elseif($status==='empty'):?><form method="post" class="v4-direct-open-form ch-direct-table-form"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="open_table"><input type="hidden" name="table_id" value="<?=(int)$t['id']?>"><button type="submit" class="ch-direct-table-button" aria-label="<?=e($t['name'])?> masasını aç"><div class="ch-table-card-v2"><div class="ch-table-card-v2__top"><span class="ch-table-card-v2__status"><?=e($statusLabel)?></span><span class="ch-table-card-v2__more" aria-hidden="true">⋮</span></div><div class="ch-table-card-v2__identity"><h3 class="ch-table-card-v2__name"><?=e($t['name'])?></h3><span class="ch-table-card-v2__area"><?=e($t['area_name'])?></span></div><div class="ch-table-card-v2__divider"></div><div class="ch-table-card-v2__metrics is-empty"><span class="ch-table-card-v2__empty-icon" aria-hidden="true">◫</span></div><span class="ch-table-card-v2__footer is-empty" aria-hidden="true">—</span></div></button></form>
...
<dialog id="discountDialog" class="v4-dialog"><form method="post" class="v4-dialog-card ch-finop-dialog"><button type="button" class="v4-dialog-x" onclick="discountDialog.close()">×</button><h2>Adisyon İskontosu</h2><p class="ch-finop-help">İskontoyu yüzde veya sabit tutar olarak uygulayın.</p><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="discount"><input type="hidden" name="session_id" value="<?=$selected?>">
<div class="ch-finop-type"><label><input type="radio" name="discount_type" value="percent" <?=($active['discount_type']??'')==='percent'?'checked':''?>><span>% Yüzde</span></label><label><input type="radio" name="discount_type" value="amount" <?=($active['discount_type']??'')!=='percent'?'checked':''?>><span>₺ Tutar</span></label></div>
<label>Değer<input type="number" min="0" step="0.01" name="discount_value" value="<?=e((string)($active['discount_value']??$financial['discount']))?>" required></label>
<label>Sebep<select name="discount_reason"><option value="">Seçiniz</option><option>Müşteri Memnuniyeti</option><option>Şikayet</option><option>Personel Hatası</option><option>Kampanya</option><option>Yetkili İndirimi</option><option>Diğer</option></select></label>
<label>Açıklama<input name="discount_note" value="<?=e($active['discount_note']??'')?>" placeholder="İsteğe bağlı not"></label>
<button class="v4-primary v4-full">İskontoyu Uygula</button></form></dialog>
<dialog id="complimentaryDialog" class="v4-dialog"><form method="post" class="v4-dialog-card ch-finop-dialog" id="complimentaryForm"><button type="button" class="v4-dialog-x" onclick="complimentaryDialog.close()">×</button><h2>Seçili Ürünleri İkram Et</h2><p class="ch-finop-help"><b data-gift-selection-label>Önce soldaki adisyondan ürün seçin.</b> İkram edilen ürün adisyondan silinmez; İKRAM olarak kalır ve fişte görünür.</p><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="complimentary_selected"><input type="hidden" name="session_id" value="<?=$selected?>"><input type="hidden" name="product_selection" data-gift-selection value="{}">
<label>Sebep<select name="complimentary_reason" required><option value="">Seçiniz</option><option>Müşteri Memnuniyeti</option><option>Şikayet</option><option>Personel Hatası</option><option>Promosyon</option><option>Yetkili İkramı</option><option>Diğer</option></select></label>
<label>Açıklama<input name="complimentary_note" placeholder="İsteğe bağlı not"></label>
<button class="v4-primary v4-full" data-gift-submit disabled>İkram Olarak İşaretle</button></form></dialog>

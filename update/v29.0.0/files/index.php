...
?><section class="category-directory" aria-labelledby="category-directory-title">
  <header class="directory-head">
    <span>MENÜMÜZ</span>
    <h2 id="category-directory-title">Bugün ne yemek istersiniz?</h2>
    <p>Kategoriyi aç, ürünleri incele ve ayrıntılar için ürüne dokun.</p>
  </header>
  ...
  <?php $catIndex=0; foreach($categories as $c):$items=$byCat[(int)$c['id']]??[];if(!$items)continue;$catIndex++;$panelId='cat-panel-'.(int)$c['id'];$titleId='cat-title-'.(int)$c['id']; ?>
    <section class="category-group" id="cat-<?=(int)$c['id']?>" style="--cat-index:<?=$catIndex?>" data-category-group>
      ...
      <div class="category-panel" id="<?=$panelId?>" role="region" aria-labelledby="<?=$titleId?>" hidden>
        ...
        <?php foreach($items as $i=>$p):$img=menu_image_url((string)($p['image_path']??''));$price=number_format((float)$p['price'],2,',','.').' '.setting('currency_symbol','₺'); ?>
          <article class="product-row"
            ...
            <?php if($uiShowImages):?><div class="product-thumb<?php if(!$img):?> is-placeholder<?php endif;?>">
              <?php if($img):?><img src="<?=e($img)?>" alt="<?=e($p['name'])?>" loading="lazy" decoding="async"><?php else:?><span><?=e(mb_substr($p['name'],0,1,'UTF-8'))?></span><?php endif;?>
            </div><?php endif;?>
            <div class="product-info">
              <h4><?=e($p['name'])?></h4>
              <?php if($uiShowDescriptions):?><?php if($p['description']):?><p><?=e($p['description'])?></p><?php else:?><p class="product-no-description">Detayları görüntülemek için dokun.</p><?php endif;?><?php endif;?>
            ...
            <?php if($uiShowRowArrow):?><span class="product-row-arrow" aria-hidden="true">›</span><?php endif;?>
          </article>
        <?php endforeach;?>
        </div>
      </div>
...
?><!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="<?=e($accent)?>"><title><?=e($cmsSeoTitle)?></title><meta name="description" content="<?=e($cmsSeoDescription)?>"><?php if($cmsCanonical):?><link rel="canonical" href="<?=e($cmsCanonical)?>"><?php endif;?><?php if($cmsOgImage):?><meta property="og:image" content="<?=e($cmsOgImage)?>"><?php endif;?><link rel="stylesheet" href="app/assets/qr-menu.css?v=500"><link rel="stylesheet" href="app/assets/qr-themes/<?=e($theme['css'])?>?v=500"><link rel="stylesheet" href="app/assets/qrx-premium-menu.css?v=2900"><style>:root{--accent:<?=e($accent)?>;--gold:<?=e($accent)?>;--paper:<?=e($uiSurface)?>;--cream:<?=e($uiBackground)?>;--ink:<?=e($uiText)?>;--muted:<?=e($uiMuted)?>;--qr-font:<?=$font?>;--qr-radius:<?=$radius?>px;--qr-hero-height:<?=$heroHeight?>vh;--qrx-detail-image-height:<?=$detailImageHeight?>px}.qrx-campaign{margin:16px auto;max-width:1100px;border-radius:var(--qr-radius);overflow:hidden;background:var(--qrx-bg);color:var(--qrx-color);box-shadow:0 14px 36px #0002}.qrx-campaign img,.qrx-campaign video{width:100%;max-height:340px;object-fit:cover;display:block}.qrx-campaign>div{padding:18px 20px}.qrx-campaign strong{font-size:clamp(20px,5vw,32px)}.qrx-campaign p{margin:8px 0}.qrx-campaign a{display:inline-block;margin-top:6px;padding:9px 14px;background:#fff;color:#111;border-radius:999px;text-decoration:none;font-weight:700}.qrx-badge{display:inline-block;margin:5px 5px 0 0;padding:4px 8px;border-radius:999px;background:var(--badge);color:#fff;font-size:11px;font-weight:800}.product-top>div{min-width:0}body.qrx-runtime{background:var(--cream);color:var(--ink)}.cms-content-block{max-width:1100px;margin:24px auto;border-radius:var(--qr-radius);overflow:hidden;background:var(--cms-bg);color:var(--cms-color);display:grid;grid-template-columns:minmax(0,1fr);box-shadow:0 16px 40px #0001}.cms-block-media img{width:100%;max-height:440px;object-fit:cover;display:block}.cms-block-copy{padding:clamp(22px,5vw,54px)}.cms-block-copy span{font-size:12px;font-weight:900;letter-spacing:.14em;text-transform:uppercase;opacity:.65}.cms-block-copy h2{font-size:clamp(28px,6vw,52px);line-height:1.05;margin:10px 0}.cms-block-copy p{font-size:16px;line-height:1.7;opacity:.82}.cms-block-copy a{display:inline-block;margin-top:10px;padding:11px 16px;border-radius:999px;background:var(--accent);color:#fff;text-decoration:none;font-weight:800}@media(min-width:800px){.cms-type-image,.cms-type-banner{grid-template-columns:1.1fr 1fr;align-items:stretch}.cms-type-image .cms-block-media img,.cms-type-banner .cms-block-media img{height:100%;max-height:none}}</style><script defer src="app/assets/qr-menu.js?v=500"></script><script defer src="app/assets/qrx-premium-menu.js?v=2900"></script></head><body class="qr-theme theme-<?=e($themeKey)?> card-<?=e($cardStyle)?> category-style-<?=e($uiCategoryStyle)?> density-<?=e($uiDensity)?> qrx-detail-<?=e($detailVariant)?>-mode <?=$uiShowImages?'show-product-images':'hide-product-images'?> <?=$qrxActive?'qrx-runtime':''?>" data-first-category-open="<?=$uiFirstOpen?'1':'0'?>" <?=$preview?'data-preview="1"':''?>><main id="menu" class="menu-wrap"><div class="qrx-progress" aria-hidden="true"><span></span></div><div class="qrx-mini-header" aria-hidden="true"><div><strong><?=e(setting('business_name','Restoran'))?></strong><span>Kategori Menüsü</span></div><a href="#menu-content" tabindex="-1">Keşfet</a></div><div id="menu-content" class="menu-anchor" aria-hidden="true"></div><?php $ctx=compact('components','categories','byCat','hero','logo','heroOverlay','showHero','showStatus','showReadonly','heroTitle','heroSubtitle','qrxActive','qrxCampaigns','qrxBadges');$runtimeOrder=QrExperience::layout((bool)$preview);foreach($runtimeOrder as $row){if(empty($row['enabled']))continue;$section=(string)$row['id'];if($section==='menu')$section='categories';elseif($section==='chef')$section='chef_note';elseif($section==='footer')continue;render_qr_section($section,$ctx);}?><?php foreach($cmsBlocks as $block):?><section class="cms-content-block cms-type-<?=e($block['block_type'])?>" style="--cms-bg:<?=e($block['background_color'])?>;--cms-color:<?=e($block['text_color'])?>"><?php if(!empty($block['media_url'])):?><div class="cms-block-media"><img src="<?=e($block['media_url'])?>" alt="<?=e($block['title']??'')?>" loading="lazy"></div><?php endif;?><div class="cms-block-copy"><?php if(!empty($block['subtitle'])):?><span><?=e($block['subtitle'])?></span><?php endif;?><?php if(!empty($block['title'])):?><h2><?=e($block['title'])?></h2><?php endif;?><?php if(!empty($block['body'])):?><p><?=nl2br(e($block['body']))?></p><?php endif;?><?php if(!empty($block['button_text'])):?><a href="<?=e($block['button_url']?:'#menu')?>"><?=e($block['button_text'])?></a><?php endif;?></div></section><?php endforeach;?>
<footer class="menu-footer"><p><?=nl2br(e(qrx_public_setting('qr_price_note','Fiyatlarımıza KDV dahildir.')))?></p><small><?=e(setting('business_name','Restoran'))?> · <?=date('Y')?></small></footer></main><button class="to-top" aria-label="Yukarı çık">↑</button>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $files = [
            'app/assets/pos-v5-beta.js',
            'app/assets/pos-waiter-ui-v2.1.js',
            'staff/index.php',
        ];
        $backup = $root . '/storage/backups/v30.5.3.1-' . date('Ymd-His');
        ...
        $status = 'Ürün kartı işlem kilidi düzeltildi.';
    } catch (Throwable $e) { $error = $e->getMessage(); }
}
?><!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>CherryHouse v30.5.3.1</title><style>body{font-family:system-ui;background:#f5f2ed;color:#182238;margin:0;padding:40px}.box{max-width:720px;margin:auto;background:#fff;padding:32px;border-radius:22px;box-shadow:0 18px 55px #16213a18}button{border:0;border-radius:13px;background:#eb6828;color:#fff;padding:14px 20px;font-weight:800;font-size:16px}.ok{background:#e8f7ef;color:#176b40;padding:14px;border-radius:12px}.err{background:#fdecec;color:#9d2424;padding:14px;border-radius:12px}code{background:#f1f3f6;padding:2px 6px;border-radius:6px}</style></head><body><div class="box"><h1>POS Ürün Dokunuşu Hotfix</h1><p>Eski çift-gönderim kilidi ile yeni hızlı ürün ekleme akışı arasındaki çakışmayı giderir.</p><?php if($status):?><p class="ok"><?=htmlspecialchars($status)?></p><p><a href="../../staff/">Garson ekranını aç</a></p><?php elseif($error):?><p class="err"><?=htmlspecialchars($error)?></p><?php endif;?><form method="post"><button type="submit">Hotfix’i Uygula</button></form></div></body></html>

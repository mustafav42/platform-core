...
final class ControlCenterRegistry
{
    ...
    public static function navigation(): array
    {
        return [
            ...
            self::group('operations', 'Operasyon', [
                self::item('business-day', 'İş Günü', '../business-day.php', '◷', 'business_day.view', null, false, ['gün başı','gün sonu']),
                self::item('tables', 'Salon ve Masalar', '../?page=tables', '▦', 'tables.manage', 'tables', false, ['masa','salon']),
            ...
            self::group('menu', 'Menü Yönetimi', [
                self::item('menu', 'Menü Merkezi', 'menu.php', '▤', 'catalog.manage', null, false, ['menü']),
            ...
            self::group('qr', 'QR Yönetimi', [
                self::item('qrx', 'QR Experience Studio', '../qr-experience/', '◈', 'catalog.manage', 'qr-menu', false, ['qr','tema','studio']),
            ...
            self::group('kitchen', 'Mutfak', [
                ...
                self::item('printer-center', 'Yazıcı Merkezi', '../printer-center.php', '▣', 'maintenance.manage', 'kitchen-printer', false, ['yazıcı']),
                self::item('print-queue', 'Yazdırma Kuyruğu', '../print-queue.php', '▤', 'maintenance.manage', 'kitchen-printer', false, ['kuyruk','baskı']),
            ], null, ['kds','kitchen-printer']),
            self::group('finance', 'Finans ve Raporlar', [
                self::item('reports', 'Rapor Merkezi', '../?page=reports', '▥', 'reports.view', 'reports', false, ['rapor','ciro','satış']),
                self::item('backup', 'Yedekleme Merkezi', '../backup.php', '⇩', 'maintenance.manage', null, false, ['yedek']),
            ]),
            self::group('management', 'Yönetim', [
                self::item('staff', 'Personel', '../?page=staff', '♙', 'staff.manage', null, false, ['personel','kullanıcı','garson']),
                self::item('permissions', 'Roller ve Yetkiler', '../permissions.php', '◈', 'permissions.manage', null, false, ['rol','yetki']),
                self::item('modules', 'Modül Merkezi', '../module-center.php', '◫', 'modules.manage', null, false, ['modül','lisans']),
                self::item('brand', 'Marka Merkezi', '../brand-center.php', '✦', 'maintenance.manage', null, false, ['logo','marka']),
                self::item('system', 'Sistem Merkezi', '../system-center.php', '●', 'maintenance.manage', null, false, ['sistem','sağlık']),
                self::item('maintenance', 'Bakım ve Yedekleme', '../?page=maintenance', '⚙', 'maintenance.manage', null, false, ['bakım']),
                self::item('security', 'Güvenlik ve Kayıtlar', '../?page=security', '◆', 'security.manage', null, false, ['güvenlik','kayıt']),
    ...
    /** @return array<string,mixed> */
    private static function item(string $id, string $label, string $href, string $icon, string $permission, ?string $module=null, bool $external=false, array $keywords=[]): array
    {
    ...
    public static function quickActions(): array
    {
        $actions=[
            ...
            self::item('quick-staff','Personel Ekle','../?page=staff&action=create','＋','staff.manage',null,false,['personel ekle']),
            self::item('quick-day','İş Günü','../business-day.php','◷','business_day.view',null,false,['gün başı gün sonu']),
            self::item('quick-backup','Yedek Al','../backup.php','⇩','maintenance.manage',null,false,['yedek al']),
            self::item('quick-modules','Modüller','../module-center.php','◫','modules.manage',null,false,['modül merkezi']),
        ];

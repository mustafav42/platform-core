    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if((string)($_POST['action']??'')==='upload'){
            ...
            foreach($files['name'] as $i=>$name){
                ...
                $uploadedIds[]=(int)ent_db()->lastInsertId();
            }
            if(count($uploadedIds)===1) ent_redirect('image-studio.php?id='.$uploadedIds[0]);
...
?><!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Medya Seç</title><link rel="stylesheet" href="assets/media-picker.css?v=3036"><style>.mp-card{position:relative}.mp-edit{position:absolute;right:8px;top:8px;z-index:2;background:#111d;color:#fff;padding:6px 9px;border-radius:8px;text-decoration:none;font-size:12px;font-weight:800}.mp-item{width:100%}</style></head><body class="mp-page">
...
<div class="mp-grid"><?php foreach($items as $item):?><div class="mp-card"><button class="mp-item" type="button" data-select-path="<?=ent_e($item['relative_path'])?>" data-select-url="<?=ent_e(ent_media_url($item['relative_path']))?>" data-select-alt="<?=ent_e($item['alt_text'])?>"><img src="<?=ent_e(ent_media_url($item['relative_path']))?>" alt=""><span><b><?=ent_e($item['original_name'])?></b><small><?=ent_e($item['folder'])?> · <?=(int)$item['width']?>×<?=(int)$item['height']?></small></span></button><a class="mp-edit" href="image-studio.php?id=<?=(int)$item['id']?>">Image Studio</a></div><?php endforeach;?><?php if(!$items):?><div class="mp-empty">Henüz görsel bulunmuyor.</div><?php endif;?></div></section>

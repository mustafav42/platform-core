...
function product_allergen_options(): array { return [
    'gluten'=>'Gluten','milk'=>'Süt','egg'=>'Yumurta','peanut'=>'Yer fıstığı','nuts'=>'Sert kabuklu yemişler',
    'soy'=>'Soya','sesame'=>'Susam','fish'=>'Balık','crustaceans'=>'Kabuklu deniz ürünleri','molluscs'=>'Yumuşakçalar',
    'mustard'=>'Hardal','celery'=>'Kereviz','lupin'=>'Acı bakla','sulphites'=>'Sülfit'
]; }
function normalize_allergens(mixed $value): string {
    $allowed=array_keys(product_allergen_options());$items=is_array($value)?$value:explode(',',(string)$value);
    $items=array_values(array_unique(array_filter(array_map(static fn($v)=>preg_replace('/[^a-z_]/','',strtolower(trim((string)$v))),$items),static fn($v)=>in_array($v,$allowed,true))));
...
try {
 if($_SERVER['REQUEST_METHOD']==='POST'){
   ...
   if($action==='save_product'){
      ...
      $allergens=normalize_allergens($_POST['allergens']??[]);
      ...
      if($id>0){
         ...
         $pdo->prepare('UPDATE products SET category_id=?,name=?,description=?,price=?,image_path=?,sort_order=?,is_active=?,calories_kcal=?,prep_time_min=?,allergen_codes=? WHERE id=?')->execute([$categoryId,$name,$description,$price,$image!==''?$image:null,$sort,$active,$calories,$prepTime,$allergens!==''?$allergens:null,$id]);
      ...
      $pdo->prepare('INSERT INTO products(category_id,name,description,price,image_path,sort_order,is_active,calories_kcal,prep_time_min,allergen_codes) VALUES(?,?,?,?,?,?,?,?,?,?)')->execute([$categoryId,$name,$description,$price,$initialImage,$sort,$active,$calories,$prepTime,$allergens!==''?$allergens:null]);
   ...
   if($action==='duplicate_product'){
      $id=(int)$_POST['id'];$q=$pdo->prepare('SELECT category_id,name,description,price,image_path,sort_order,calories_kcal,prep_time_min,allergen_codes FROM products WHERE id=?');$q->execute([$id]);$p=$q->fetch();if(!$p)throw new RuntimeException('Ürün bulunamadı.');
      $pdo->prepare('INSERT INTO products(category_id,name,description,price,image_path,sort_order,is_active,calories_kcal,prep_time_min,allergen_codes) VALUES(?,?,?,?,?,?,1,?,?,?)')->execute([(int)$p['category_id'],$p['name'].' (Kopya)',$p['description'],$p['price'],$p['image_path'],(int)$p['sort_order']+1,$p['calories_kcal']??null,$p['prep_time_min']??null,$p['allergen_codes']??null]);cat_redirect('products','Ürün kopyalandı.');
   }
...
<?php if($tab==='products'): $p=$editProduct?:['id'=>0,'category_id'=>'','name'=>'','description'=>'','price'=>'','image_path'=>'','sort_order'=>0,'is_active'=>1,'calories_kcal'=>'','prep_time_min'=>'','allergen_codes'=>''];?><form method="post" enctype="multipart/form-data"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="save_product"><input type="hidden" name="id" value="<?=(int)$p['id']?>"><div class="form-grid"><label>Kategori<select name="category_id" required><option value="">Kategori seçin</option><?php foreach($categories as $c):?><option value="<?=$c['id']?>" <?=(int)$p['category_id']===(int)$c['id']?'selected':''?>><?=e($c['name'])?></option><?php endforeach;?></select></label><label>Ürün adı<input name="name" value="<?=e((string)$p['name'])?>" required></label><label>Fiyat<input name="price" type="number" min="0" step="0.01" value="<?=e((string)$p['price'])?>" required></label><label>Sıra<input name="sort_order" type="number" value="<?=(int)$p['sort_order']?>"></label><label class="wide">Açıklama<textarea name="description" rows="4"><?=e((string)$p['description'])?></textarea></label><label class="wide">Ürün görseli<input data-media-picker name="image_path" value="<?=e((string)$p['image_path'])?>"><span>Ortam Kütüphanesinden seçebilir veya aynı pencerede yeni görsel yükleyebilirsin.</span></label><label class="wide upload">Bilgisayardan doğrudan yükle<input id="imageInput" name="image" type="file" accept="image/jpeg,image/png,image/webp"><span>JPG, PNG veya WebP · En fazla 32 MB</span></label><?php if($p['image_path']):?><div class="current-image wide"><img src="../<?=e((string)$p['image_path'])?>"><label><input type="checkbox" name="remove_image" value="1"> Mevcut görseli kaldır</label></div><?php endif;?><div id="previewWrap" class="current-image wide" hidden><img id="preview" alt="Önizleme"></div>
<fieldset class="product-info-fields wide"><legend>Ürün Bilgileri</legend><p>Bu alanlar yalnızca bilgi girildiğinde QR ürün detayının en altında sade biçimde görünür.</p><div class="nutrition-grid"><label>Kalori (kcal)<input name="calories_kcal" type="number" min="0" max="9999" value="<?=e((string)($p['calories_kcal']??''))?>" placeholder="Örn. 540"></label><label>Hazırlama süresi<select name="prep_time_min"><option value="">Gösterme</option><?php foreach([5,10,15,20,25,30,45,60] as $minute):?><option value="<?=$minute?>" <?=((string)($p['prep_time_min']??''))===(string)$minute?'selected':''?>><?=$minute===60?'60 dk+':$minute.' dk'?></option><?php endforeach;?></select></label></div><div class="allergen-title">Alerjenler</div><div class="allergen-grid"><?php $selectedAllergens=array_filter(explode(',',(string)($p['allergen_codes']??'')));foreach(product_allergen_options() as $code=>$label):?><label class="allergen-check"><input type="checkbox" name="allergens[]" value="<?=e($code)?>" <?=in_array($code,$selectedAllergens,true)?'checked':''?>><span><?=e($label)?></span></label><?php endforeach;?></div></fieldset>
<label class="switch wide"><input type="checkbox" name="is_active" value="1" <?=$p['is_active']?'checked':''?>><span></span> Aktif olarak yayınla</label></div><div class="dialog-actions"><a class="secondary" href="catalog.php?tab=products">İptal</a><button class="primary">Kaydet</button></div></form>

...
function upload_product_image(array $file): ?string {
    ...
    if((int)($file['size']??0)>8*1024*1024) throw new RuntimeException('Görsel en fazla 8 MB olabilir.');
...
try {
 if($_SERVER['REQUEST_METHOD']==='POST'){
   ...
   if($action==='save_product'){
      ...
      $pdo->prepare('INSERT INTO products(category_id,name,description,price,image_path,sort_order,is_active) VALUES(?,?,?,?,?,?,?)')->execute([$categoryId,$name,$description,$price,$newImage,$sort,$active]);
...
$msg=(string)($_GET['msg']??'');
?>
<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Menü Yönetimi</title><link rel="stylesheet" href="assets/catalog.css?v=2840"></head>

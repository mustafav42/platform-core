...
try{
 if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['action'])){
  ...
  if($action==='open_table'){
   ...
   $newId=(int)$pdo->lastInsertId();$pdo->prepare("UPDATE restaurant_tables SET status='open' WHERE id=?")->execute([$tableId]);$pdo->commit();redirect('./?session='.$newId);
  }
  if($action==='stage_item'){
   ...
   $_SESSION['waiter_pending_orders'][$sid]=$cart;redirect('./?session='.$sid.'&category='.$categoryId);
  }
...
?><!doctype html>
<html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>CherryHouse POS · Garson</title><link rel="stylesheet" href="../app/assets/pos-v4.css?v=420"><link rel="stylesheet" href="../app/assets/cherry-design/tokens.css?v=300"><link rel="stylesheet" href="../app/assets/pos-v5-alpha.css?v=3010"><link rel="stylesheet" href="../app/assets/table-flow-hotfix.css?v=2701"><script defer src="../app/assets/pos-v4.js?v=420"></script><script defer src="../app/assets/pos-v5-alpha.js?v=3010"></script></head>
<body class="chv4 chv4-waiter ch-pos5">
<header class="v4-topbar"><a class="v4-back" href="<?=$active?'./':'../admin/'?>">‹</a><div class="v4-brand"><span class="v4-mark">🍒</span><div><strong><?=e(setting('business_name','CherryHouse'))?></strong><small>POS 5.0 Alpha · Garson</small></div></div><div class="v4-top-status"><span class="online-dot"></span> Çevrimiçi <b class="v4-live-clock" data-live-clock></b></div><button type="button" class="v4-fullscreen" data-fullscreen title="Tam ekran">⛶</button><div class="v4-user"><span><?=e($_SESSION['staff_name'])?></span><a href="?logout=1">Çıkış</a></div></header>
...
<main class="v4-table-layout">
 ...
 <div class="v4-table-grid ch-table-grid" data-table-grid><?php foreach($tables as $t):$status=(string)$t['status'];?><article class="v4-table-card <?=!empty($t['session_id'])?'is-open':'is-'.$status?>" <?php if(!empty($t['opened_at'])):?>data-opened="<?=e((string)$t['opened_at'])?>"<?php endif;?> data-area="area-<?=(int)$t['area_id']?>" data-table-name="<?=e(mb_strtolower((string)$t['name']))?>">
 <?php if($t['session_id']):?><a href="?session=<?=(int)$t['session_id']?>"><div class="v4-table-top"><span class="v4-table-pill">DOLU</span><small><span data-open-duration>-- dk</span> · <?=date('H:i',strtotime((string)$t['opened_at']))?></small></div><h3><?=e($t['name'])?></h3><div class="v4-table-meta"><span>👤 <?=(int)$t['guest_count']?></span><span><?=e($t['area_name'])?></span></div><strong class="v4-table-total"><?=number_format((float)$t['table_total'],2,',','.')?> ₺</strong></a>
 <?php elseif($status==='empty'):?><form method="post" class="ch-direct-table-form"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="open_table"><input type="hidden" name="table_id" value="<?=(int)$t['id']?>"><button type="submit" class="ch-direct-table-button"><div class="v4-table-top"><span class="v4-table-pill">BOŞ</span></div><h3><?=e($t['name'])?></h3><div class="v4-table-meta"><span><?=e($t['area_name'])?></span></div><strong class="v4-open-hint">Siparişe geç</strong></button></form>
 <?php else:?><div><div class="v4-table-top"><span class="v4-table-pill"><?=e(mb_strtoupper($status))?></span></div><h3><?=e($t['name'])?></h3><div class="v4-table-meta"><span><?=e($t['area_name'])?></span></div></div><?php endif;?></article><?php endforeach;?></div></section>
</main>
<?php if($active):?>
...
<main class="v4-order-layout">
 <aside class="v4-category-rail"><div class="v4-rail-head"><a href="./">‹ Masalar</a><small>KATEGORİLER</small></div><nav><?php foreach($categories as $c):?><a class="<?=$categoryId===(int)$c['id']?'active':''?>" href="?session=<?=$sessionId?>&category=<?=(int)$c['id']?>"><?=e($c['name'])?></a><?php endforeach;?></nav></aside>
 <section class="v4-product-zone"><div class="v4-work-head"><div><span class="v4-eyebrow"><?=e($active['area_name'])?></span><h1><?=e($active['table_name'])?></h1></div><label class="v4-search"><span>⌕</span><input type="search" placeholder="Ürün ara..." data-product-search></label></div><div class="v4-mobile-view-switch" data-mobile-switch><button type="button" class="active" data-mobile-view="products">Ürünler</button><button type="button" data-mobile-view="ticket">Adisyon <span><?=count($items)+count($pending)?></span></button></div><div class="v4-product-grid"><?php foreach($products as $p):?><form method="post" class="v4-product-form" data-product-name="<?=e(mb_strtolower($p['name']))?>" data-quick-product data-product-label="<?=e($p['name'])?>"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="stage_item"><input type="hidden" name="session_id" value="<?=$sessionId?>"><input type="hidden" name="product_id" value="<?=(int)$p['id']?>"><input type="hidden" name="quantity" value="1" data-quick-quantity><input type="hidden" name="item_note" value="" data-quick-note><button type="submit" class="v4-product-card" data-quick-trigger><span class="v4-qty-badge" data-quick-badge hidden>1</span><div class="v4-product-image"<?php if($p['image_path']):?> style="background-image:url('../<?=e(ltrim((string)$p['image_path'],'/'))?>')"<?php endif;?>><?=$p['image_path']?'':'🍽️'?></div><div><strong><?=e($p['name'])?></strong><b><?=number_format((float)$p['price'],2,',','.')?> ₺</b></div><small class="v4-hold-hint">Basılı tut: not</small></button></form><?php endforeach;?></div></section>
 <aside class="v4-ticket"><header><div><span class="v4-eyebrow">ADİSYON</span><h2><?=e($active['table_name'])?></h2><small><?=(int)$active['guest_count']?> kişi · <?=date('H:i',strtotime((string)$active['opened_at']))?></small></div><span class="v4-ticket-count"><?=count($items)+count($pending)?></span></header><div class="v4-ticket-body"><h3>Gönderilenler</h3><?php if(!$items):?><p class="v4-empty-text">Henüz gönderilmiş ürün yok.</p><?php endif;?><?php foreach($items as $it):?><div class="v4-line committed"><div><b><?=e($it['quantity'])?>×</b><span><strong><?=e($it['product_name'])?></strong><small><?=e($it['staff_name']??'')?><?php if($it['item_note']):?> · 📝 <?=e($it['item_note'])?><?php endif;?></small></span></div><em><?=number_format((float)$it['unit_price']*(float)$it['quantity'],2,',','.')?> ₺</em></div><?php endforeach;?><h3 class="new">Yeni Sipariş <span><?=count($pending)?></span></h3><?php if(!$pending):?><p class="v4-empty-text">Ürünlere dokunarak ekleyin.</p><?php endif;?><?php foreach($pending as $row):?><div class="v4-line pending"><div><b><?=e($row['quantity'])?>×</b><span><strong><?=e($row['product_name'])?></strong><small><?=number_format((float)$row['unit_price'],2,',','.')?> ₺<?php if($row['item_note']):?> · 📝 <?=e($row['item_note'])?><?php endif;?></small></span></div><em><?=number_format((float)$row['unit_price']*(float)$row['quantity'],2,',','.')?> ₺</em><div class="v4-line-actions"><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="pending_qty"><input type="hidden" name="session_id" value="<?=$sessionId?>"><input type="hidden" name="key" value="<?=e($row['key'])?>"><input type="hidden" name="delta" value="-1"><button>−</button></form><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="pending_qty"><input type="hidden" name="session_id" value="<?=$sessionId?>"><input type="hidden" name="key" value="<?=e($row['key'])?>"><input type="hidden" name="delta" value="1"><button>+</button></form><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="remove_pending"><input type="hidden" name="session_id" value="<?=$sessionId?>"><input type="hidden" name="key" value="<?=e($row['key'])?>"><button class="danger">×</button></form></div></div><?php endforeach;?></div><footer><div class="v4-ticket-totals"><span>Mevcut <b><?=number_format($total,2,',','.')?> ₺</b></span><span>Yeni <b><?=number_format(waiterCartTotal($pending),2,',','.')?> ₺</b></span><strong>Toplam <em><?=number_format($total+waiterCartTotal($pending),2,',','.')?> ₺</em></strong></div><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="submit_order"><input type="hidden" name="session_id" value="<?=$sessionId?>"><button class="v4-primary v4-full" <?=$pending?'':'disabled'?>>Siparişi Gönder</button></form><?php if($pending):?><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="discard_pending"><input type="hidden" name="session_id" value="<?=$sessionId?>"><button class="v4-ghost v4-full">Seçimleri Temizle</button></form><?php endif;?></footer></aside>
</main><?php endif;?>

...
<main class="v4-table-layout">
 ...
 <div class="v4-table-grid ch-table-grid" data-table-grid><?php foreach($tables as $t):$status=(string)$t['status'];?><article class="v4-table-card <?=!empty($t['session_id'])?'is-open':'is-'.$status?>" <?php if(!empty($t['opened_at'])):?>data-opened="<?=e((string)$t['opened_at'])?>"<?php endif;?> data-area="area-<?=(int)$t['area_id']?>" data-table-id="<?=(int)$t['id']?>" data-session-id="<?=(int)($t['session_id']??0)?>" data-table-name="<?=e(mb_strtolower((string)$t['name']))?>">

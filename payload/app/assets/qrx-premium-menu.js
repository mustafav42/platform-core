...
function openDetail(row){
  ...
  const imageFallback=row.dataset.imageFallback||row.querySelector('.product-thumb img')?.currentSrc||row.querySelector('.product-thumb img')?.src||'';
  ...
  const media=modal.querySelector('.qrx-detail-media');
  if(media){
    ...
    if(image||imageFallback){
      ...
      detailImage.src=image||imageFallback;
      detailImage.addEventListener('error',()=>{
        if(imageFallback&&detailImage.src!==new URL(imageFallback,document.baseURI).href){detailImage.src=imageFallback;return;}
        const placeholder=document.createElement('div');
        placeholder.className='qrx-detail-placeholder';
        placeholder.textContent=name.slice(0,1);
        media.replaceChildren(placeholder);
      },{once:false});
      media.appendChild(detailImage);
    }else{
      const placeholder=document.createElement('div');
      placeholder.className='qrx-detail-placeholder';
      placeholder.textContent=name.slice(0,1);
